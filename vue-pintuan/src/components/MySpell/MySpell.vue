<template>
    <div class="mySpell">
      <mt-header :fixed="true" class="topBar" style="font-size: 0.4rem" title="我的砍价">
      ...
      <scroll class="scroll" ref="scroll" :probeType="3" :listenScroll="true" @scroll="getNewData">
        <div>
          <mySpellListItem @continuePay="continuePay(item)" :key="index" v-for="(item,index) in list" :data="item"></mySpellListItem>
...
<script type='text/ecmascript-6'>
  ...
  import mySpellListItem from '@/common/components/mySpellListItem'
  ...
  import  continuePay from '../../common/components/continuePay.vue'
    export default {
      ...
      computed:{
        ...mapGetters(['siteId','token','user'])
      },
        components:{
          Scroll,mySpellListItem,continuePay
        }
    }
</script>
