        ...
        <!--  <div class="item" v-if="item.id==6992" v-for="item in list">
          ...
          <div class="item"  v-for="item in list" :key="item.id">
            <div class="info">
              ...
              <div class="productInfo">
                <div class="price" style="color:black">市场价：<i style="text-decoration: line-through;margin-right: 0.2rem">{{item.oldPrice | fulterUnit}}</i><i style="font-size:0.5rem;color:red">{{item.price | fulterUnit}}</i></div>
                <div class="consumption">符合资格 {{item.num}}人</div>
                <div class="btn" v-if="!item.success" @click="spell(item)">邀请好友帮我助力</div>
                <!-- <div class="btn" v-if="item.success && item.vo.remarks != 1" @click="getGift(item)">领取奖品</div>-->
                <div class="btn" v-if="item.success" @click="$router.push('/myGift6')">填写资料参与抽奖</div>
              </div>
            </div>
            <div v-if="item.vo">
              <div class="load" >
   ...
   import {mapGetters} from 'vuex'
    ...
    export default {
      ...
      methods: {
          ...
          share(item,id){
            window.location.href = `http://cmbls/socialshare?id=mpBank&&channel=defaultWithNoSMS&type=url&title=0元限时瓜分壕礼，海量礼包大派送！&text=${item.name}&picurl=${item.photo}&url=${this.AuthConfig.authShorUrl}?zlid6=${id}`
          },
          spell(item){
            let product = item
            //console.log(item)
